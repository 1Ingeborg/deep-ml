...
def compute_cross_entropy_loss(predicted_probs: torch.Tensor, true_labels: torch.Tensor, epsilon: float = 1e-15) -> float:
    ...
    """
    probs = torch.clamp(predicted_probs, min=epsilon, max=1.0 - epsilon)  # Avoid log(0)
    p = (probs * true_labels).sum(dim=-1)  # Extract the predicted probabilities for the true classes
    loss = -p.log()
